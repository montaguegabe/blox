...
// Constructor
Tessellation::Tessellation() { mMode = TessellationSquare; }
...
// Calculates a snapped motion vector from a starting position and a raw motion
// vector
MVector Tessellation::snapMotionVector(MVector &input) {
  ...
  case TessellationSquare: {
    ...
    else if (angle >= 3.0 * M_PI_4 || angle < -3.0 * M_PI_4)
...
// Mode access
void Tessellation::setMode(TessellationMode mode) { mMode = mode; }
TessellationMode Tessellation::getMode() { return mMode; }