...
MVector getCameraVector(MStatus& stat) {
    ...
    if (!stat) { stat.perror("Error getting camera vector"); return MVector::zero; }
...
MStatus translateSelection(MVector translation) {
    ...
    for (MItSelectionList listIter(list, MFn::kTransform); !listIter.isDone(); listIter.next()) {
        ...
        if (stat) transformFn.translateBy(translation, MSpace::kWorld);
        else {
            MGlobal::displayError("Couldn't operate transformations on selection");
            return stat;
        }
    }
